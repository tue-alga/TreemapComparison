...
# Run his techniques
techniques='snd sqr otpbm otbpsize otbsplit spiral strip split hilb moore appr ssv ssv2'
for technique in $techniques
do
  complete_output_dir="$output_dir""/""$technique""/""$dataset_name" # Concatenate technique to output path
  echo $complete_output_dir
  mkdir $complete_output_dir
  java -classpath "libraries/opencsv-3.7.jar:libraries/Jama-1.0.3.jar:src" UserControl/Simulator -technique $technique -inputfolder$temp_dir -baseline true -outputfolder $complete_output_dir -width $width -height $height
done
